...
bool InterruptHandler::sys_waitpid(uint32_t esp){
    return interruptManager->taskManager->WaitTask(esp);
}
common::uint32_t InterruptHandler::sys_exec(uint32_t entrypoint){ // function was void pointer but we get it as u32int
    return interruptManager->taskManager->ExecTask((void (*)())entrypoint); // Send it back as void pointer
}
