...
void HexVector::reset(){
	...
	for( int i = 0; i< boardSize; i++){ 
		...
		for(int j = 0; j< boardSize; j++){
			...
			Cell temp_cell;
			temp_cell.setState(empty); // Filling the temporary cell with empty states.
			temp_cell.setRow(i);
			temp_cell.setCol(j);

			temp.push_back(temp_cell);
		} 
		hexCells.push_back(temp); // Filling the board with empty cells.
	}
}
...
int HexVector::play(Cell &temp_cell){
	...
	try{
		if( this->turn == user1){
				...
				if((*this)(row-1 , col-97) == empty && (row-1) <= boardSize && (col-97) < boardSize){ // Is the move valid ?
					...
					string x = col + to_string(row); // Memorizing the move.
					moves.push_back(x);
					move_number++;
			
					hexCells[row - 1][col- 97].setState(u1); // Playing
					this->turn = user2;
			...
			if((*this)(row-1 , col-97) == empty && (row-1) <= boardSize && (col-97) < boardSize){
				...
				hexCells[row - 1][col- 97].setState(u2); // Playing the move.
				this->turn = user1;

				Reset_Buffer();

				for(b = 0; b<boardSize; b++){ // Start of the winning path.
...
char HexVector::operator()(int i , int j){
...
bool HexVector::isEnd(int row , int col , char state){
	try{
		if(state== u1){
			/* FINISH LINE */ 
	...
	try{
		for(int i = 1; i<boardSize;i++){
			...
			for( int j = 1; j<boardSize; j++){
				...
				if((*this)(i,j) == u1){ // Finding a 'x' to play the move next to it.
				...
				else{ // if there is no 'x' in the board, playing the first empty place.
...
void HexVector::showWinningBoard(){
...
int HexVector::playGame(){
	...
	while(1){
		...
		getCommand(command , file); // took the move
		...
		if(file == "empty"){
			...
			if(command == "EXIT"){
				...
				temp_cell.setCol(command[0]);  // sent the move to the temporary cell to play.
		...
		if(user_or_bot(UserOrBot) == 1){	
			...
			tempp = play(temp_cell); // Human's move.

			if(tempp == 4){ // The game finished. Human won.
				return 0 ;
			}
			if(tempp == 5){ // The move wasn't valid.
				continue;
			}
			this->turn = user1;
			...
			for(int b = 0; b<boardSize; b++){ // Finding the winning path of bot.
				if((*this)(0 , b) == u2){
...
void HexVector::writeToFile(string &file, int turn){
...
void HexVector::readFromFile(string &file , int &turn , int &UserOrBot){
	...
	if(secFile.is_open()){
		...
		for(int i = 0; i< moves.size(); i++){
			...
			string str;
			str = a.substr(1);// a[0] is the column and the rest is the row. This line takes the rest to str.

			if( a[0] - 97 < boardSize && stoi(str)-1 < boardSize && a[0] - 97 > -1 && stoi(str)-1 > -1){ // is the move valid.
				if(i%2 != 0){ // If the move number is even it is 'x' move.
					moves[i] = a;
					hexCells[ stoi(str)-1][a[0] - 97].setState(u2); // o
				}
				else{
					moves[i] = a;
					hexCells[stoi(str)-1][a[0] - 97].setState(u1); // x
				}
			}
