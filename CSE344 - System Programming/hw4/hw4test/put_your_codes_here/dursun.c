...
void* threadEx(){
    ...
    while (interruptCond == 1)
    {
        sem_wait(&full);
        pthread_mutex_lock(&mutex);

        if (tail == head && exitCond == 1)
        {
...
void open_files_in_dest(char source[256], char dest[256]){
    ...
    while ((entry = readdir(dir)) != NULL && interruptCond == 1) {
...
int main(int argc, char *argv[]){
    ...
    buff = malloc(bufferSize * sizeof(struct buffer));
    head = 0; 
    tail = 0;
    ...
    workerThds = malloc(workerNum * sizeof(pthread_t));

    params = malloc(sizeof(struct manager_thread_params));
