...
int main(int argc, char *argv[]){
    ...
    pthread_barrier_init(&barrier, NULL, workerNum + 1);
    ...
    strcpy(params->source, argv[3]);
    strcpy(params->destination, argv[4]);

    pthread_create(&managerThd, NULL, manager_func, params);

    for (int i = 0; i < workerNum; i++)
    {
        pthread_create(&workerThds[i], NULL, worker, NULL);
    }
